...
hparams = {
	...
	"n_rnn_layers": 5,
	"rnn_dim": 512,
	"n_class": 29,		# 28 chars + 1 blank
	"n_feats": 80,		# input Mel filterbank features (bins)
	"stride": 2,
	"dropout": 0.1,
	"learning_rate": 5e-4, 
	"batch_size": 64, 
...
def gridSearch(model, device, val_loader, alphas, betas, kenlm_path='wiki-interpolate.3gram.arpa'):
    ### cache model outputs over the validation set
	# -> do expensive model forward pass once
	# -> do cheap decoding via weighted beam search for grid search
    model.eval()
    cached = []   
    with torch.no_grad():
        for spectrograms, labels, input_lengths, label_lengths in val_loader:
            spectrograms = spectrograms.to(device)
            output = model(spectrograms)            # (B, T, C)

            for i in range(output.shape[0]):
                logits = output[i].cpu().numpy()
                target = intToStr(labels[i][:label_lengths[i]].tolist())
                cached.append((logits, target))
    print(f'Cached {len(cached)} utterances')		# list of (logits_np, target_string)
    ...
    for alpha in alphas:
        for beta in betas:
            ...
            wers = [wer(target, decoder.decode(logits)) for logits, target in cached]
            avg_wer = sum(wers) / len(wers)

            results[(alpha, beta)] = avg_wer
            print(f'  alpha={alpha:.2f}, beta={beta:.2f}  →  WER={avg_wer:.4f}')
            if avg_wer < best[1]:
                best = ((alpha, beta), avg_wer)

    print(f'\nBest: alpha={best[0][0]}, beta={best[0][1]}, WER={best[1]:.4f}')
    return best, results
